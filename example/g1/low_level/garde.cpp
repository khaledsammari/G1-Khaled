 ...
 public:
  ...
  void Control() {
    ...
    if (ms) {
      ...
      motor_command_tmp.q_target.at(LeftHipPitch) = 0.1;
      motor_command_tmp.q_target.at(LeftKnee) = -0.2;
      motor_command_tmp.q_target.at(LeftAnklePitch) = 0.1;
      
      motor_command_tmp.q_target.at(RightHipPitch) = 0.1;
      motor_command_tmp.q_target.at(RightKnee) = -0.2;
      motor_command_tmp.q_target.at(RightAnklePitch) = 0.1;

      // Waist position (slightly forward)
      motor_command_tmp.q_target.at(WaistPitch) = -0.1;

      // Left Arm (guard position - high guard)
      motor_command_tmp.q_target.at(LeftShoulderPitch) = 0.8;   // Arm forward
      motor_command_tmp.q_target.at(LeftShoulderRoll) = 0.5;    // Arm slightly out
      motor_command_tmp.q_target.at(LeftElbow) = -1.2;          // Elbow bent
      motor_command_tmp.q_target.at(LeftWristRoll) = 0.0;       // Wrist straight

      // Right Arm (ready to punch - chambered position)
      motor_command_tmp.q_target.at(RightShoulderPitch) = 0.6;  // Arm forward
      motor_command_tmp.q_target.at(RightShoulderRoll) = -0.3;  // Arm tucked in
      motor_command_tmp.q_target.at(RightElbow) = -1.0;         // Elbow bent
      motor_command_tmp.q_target.at(RightWristRoll) = 0.0;      // Wrist straight
