 ...
 public:
  ...
  void Control() {
    ...
    if (ms) {
      ...
      // Front kick sequence parameters
      double prep_duration = 1.0;      // Preparation phase (lifting leg)
      double kick_duration = 0.5;      // Kick extension phase
      double retract_duration = 0.5;   // Retraction phase
      double reset_duration = 1.0;     // Reset to standing position
      double total_duration = prep_duration + kick_duration + retract_duration + reset_duration;
      
      if (time_ < prep_duration) {
        // [Stage 1]: Prepare for kick - lift the leg
        double ratio = std::clamp(time_ / prep_duration, 0.0, 1.0);
        
        // Lift the leg by flexing hip and knee
        motor_command_tmp.q_target.at(RightHipPitch) = -0.5 * ratio;  // Negative for lifting (forward)
        motor_command_tmp.q_target.at(RightKnee) = 1.5 * ratio;       // Positive for bending
        
        // Keep other leg joints in neutral position
        motor_command_tmp.q_target.at(RightHipRoll) = 0.0;
        motor_command_tmp.q_target.at(RightHipYaw) = 0.0;
        motor_command_tmp.q_target.at(RightAnklePitch) = 0.0;
        motor_command_tmp.q_target.at(RightAnkleRoll) = 0.0;
        
      } else if (time_ < prep_duration + kick_duration) {
        // [Stage 2]: Execute kick - extend the leg forward
        double t = time_ - prep_duration;
        double ratio = std::clamp(t / kick_duration, 0.0, 1.0);
        
        // Extend the leg forward
        motor_command_tmp.q_target.at(RightHipPitch) = -0.5 + (-0.5 * ratio);  // Continue forward
        motor_command_tmp.q_target.at(RightKnee) = 1.5 - (1.5 * ratio);        // Extend knee
        
      } else if (time_ < prep_duration + kick_duration + retract_duration) {
        // [Stage 3]: Retract the leg
        double t = time_ - (prep_duration + kick_duration);
        double ratio = std::clamp(t / retract_duration, 0.0, 1.0);
        
        // Return to lifted position
        motor_command_tmp.q_target.at(RightHipPitch) = -1.0 + (0.5 * ratio);  // Back to lifted position
        motor_command_tmp.q_target.at(RightKnee) = 0.0 + (1.5 * ratio);        // Back to bent position
        
      } else if (time_ < total_duration) {
        // [Stage 4]: Return to standing position
        double t = time_ - (prep_duration + kick_duration + retract_duration);
        double ratio = std::clamp(t / reset_duration, 0.0, 1.0);
        
        // Return all joints to zero position
        motor_command_tmp.q_target.at(RightHipPitch) = -0.5 * (1.0 - ratio);
        motor_command_tmp.q_target.at(RightKnee) = 1.5 * (1.0 - ratio);
        
      } else {
        // Reset time to loop the kick motion
        time_ = 0.0;
      }
...
int main(int argc, char const *argv[]) {
  if (argc < 2) {
    std::cout << "Usage: g1_front_kick_example network_interface" << std::endl;
    exit(0);
  }
